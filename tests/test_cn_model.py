...
def test_cn_model():
    ...
    model = CNModel(data, n_clouds=1, molecule="CN", mol_data=_MOL_DATA, baseline_degree=1)
    assert isinstance(model.mol_data, dict)
    with pytest.raises(ValueError):
        model.add_priors(assume_LTE=True, assume_CTEX=False)
    model.add_priors(
        prior_baseline_coeffs={"observation": [1.0, 1.0]}, prior_rms={"observation": 1.0}, prior_fwhm_nonthermal=1.0
    )
