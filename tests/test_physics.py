...
def test_calc_thermal_fwhm():
    fwhm = physics.calc_thermal_fwhm(100.0, 26.0).eval()
...
def test_calc_line_profile():
    ...
    assert_allclose(
        line_profile.sum(axis=0) * (freq_axis[1] - freq_axis[0]), np.ones((2, 2))
    )
...
def test_calc_optical_depth():
    ...
    optical_depth = physics.calc_optical_depth(
        _MOL_DATA, N, cloud_tex, component_tex, line_profile
    ).eval()
...
def test_predict_tau():
    ...
    tau = physics.predict_tau(
        _MOL_DATA, freq_axis, log10_N, velocity, fwhm, cloud_tex, component_tex
    ).eval()
